/**
 * Server-action tests for /lake/catalog-drift/actions.ts —
 ...
 *
 * Concern: L2's reject reason enum is L2-distinct. ``expected_change``
 * is the 5th value, replacing L1's ``duplicate``, L8's
 * ``wrong_pairing``, L6's ``wrong_level``, L5's ``wrong_type``, L4's
 * ``already_handled``, and L7's ``wrong_threshold``.
 */
import { describe, expect, it } from "vitest";

import { __test__ } from "../../app/(app)/lake/catalog-drift/actions";

const { VALID_REJECT_REASONS, readBase, readToken } = __test__;

describe("/lake/catalog-drift actions — reject reasons enum", () => {
  it("accepts only the 5 canonical L2 reasons (false_positive / inconsequential / expected_change / out_of_scope / other)", () => {
    expect(VALID_REJECT_REASONS.has("false_positive")).toBe(true);
    expect(VALID_REJECT_REASONS.has("inconsequential")).toBe(true);
    expect(VALID_REJECT_REASONS.has("expected_change")).toBe(true);
  ...
  it("rejects L1-/L3-/L4-/L5-/L6-/L7-/L8-specific reasons (defensive — L2 enum is distinct)", () => {
    // L1: duplicate + low_value; L3: wrong_direction + low_confidence;
    // L4: already_handled; L5: wrong_type; L6: wrong_level;
    // L7: wrong_threshold; L8: wrong_pairing. None belong on the
    // L2 catalog-drift surface — L2's distinctive 5th value is
    // ``expected_change``.
    expect(VALID_REJECT_REASONS.has("duplicate")).toBe(false);
    expect(VALID_REJECT_REASONS.has("low_value")).toBe(false);
    ...
    expect(VALID_REJECT_REASONS.has("wrong_pairing")).toBe(false);
  });

  it("rejects unknown reasons", () => {
    expect(VALID_REJECT_REASONS.has("totally_wrong")).toBe(false);
    expect(VALID_REJECT_REASONS.has("")).toBe(false);
    expect(VALID_REJECT_REASONS.has("  expected_change  ")).toBe(false);
  });
});

describe("/lake/catalog-drift actions — env-knob readers", () => {
