/**
 * Server-action tests for /lake/column-classification/actions.ts —
 * L6 Sub-wave D.
 *
 ...
 *
 * Concern #1: L6's reject reason enum is L6-distinct. ``wrong_level``
 * is the 5th value, replacing L5's ``wrong_type``, L4's
 * ``already_handled``, and L7's ``wrong_threshold``.
 */
import { describe, expect, it } from "vitest";

import { __test__ } from "../../app/(app)/lake/column-classification/actions";

const { VALID_REJECT_REASONS, readBase, readToken } = __test__;

describe("/lake/column-classification actions — reject reasons enum", () => {
  it("accepts only the 5 canonical L6 reasons (false_positive / low_value / wrong_level / out_of_scope / other)", () => {
    expect(VALID_REJECT_REASONS.has("false_positive")).toBe(true);
    expect(VALID_REJECT_REASONS.has("low_value")).toBe(true);
    expect(VALID_REJECT_REASONS.has("wrong_level")).toBe(true);
  ...
  it("rejects L3-only, L4-only, L5-only, and L7-only reasons (defensive — L6 enum is distinct)", () => {
    // L3: wrong_direction + low_confidence; L4: already_handled;
    // L5: wrong_type; L7: wrong_threshold. None belong on the L6
    // column-classification surface.
...
describe("/lake/column-classification actions — env-knob readers", () => {
