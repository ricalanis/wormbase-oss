/**
 * Server-action tests for /lake/schema-impact/actions.ts — L4 Sub-wave D.
 *
...
import { __test__ } from "../../app/(app)/lake/schema-impact/actions";

const { VALID_REJECT_REASONS, readBase, readToken } = __test__;

describe("/lake/schema-impact actions — reject reasons enum", () => {
  it("accepts only the 5 canonical L4 reasons (false_positive / already_handled / low_value / out_of_scope / other)", () => {
    expect(VALID_REJECT_REASONS.has("false_positive")).toBe(true);
    expect(VALID_REJECT_REASONS.has("already_handled")).toBe(true);
  ...
  it("rejects L3-only and L7-only reasons (defensive — L4 enum is distinct)", () => {
    // L3 ships wrong_direction + low_confidence; L7 ships wrong_threshold.
    // None of those belong on the L4 schema-impact surface.
...
describe("/lake/schema-impact actions — env-knob readers", () => {
