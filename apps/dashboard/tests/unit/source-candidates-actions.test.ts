/**
 * Server-action tests for /lake/source-candidates/actions.ts —
 * L1 Sub-wave D (2026-06-08).
 *
 ...
 *
 * Concern: L1's reject reason enum is L1-distinct. ``duplicate`` is
 * the 5th value, replacing L8's ``wrong_pairing``, L6's
 * ``wrong_level``, L5's ``wrong_type``, L4's ``already_handled``, and
 * L7's ``wrong_threshold``.
 */
import { describe, expect, it } from "vitest";

import { __test__ } from "../../app/(app)/lake/source-candidates/actions";

const { VALID_REJECT_REASONS, readBase, readToken } = __test__;

describe("/lake/source-candidates actions — reject reasons enum", () => {
  it("accepts only the 5 canonical L1 reasons (duplicate / false_positive / low_value / out_of_scope / other)", () => {
    expect(VALID_REJECT_REASONS.has("duplicate")).toBe(true);
  ...
  it("rejects L3-/L4-/L5-/L6-/L7-/L8-specific reasons (defensive — L1 enum is distinct)", () => {
    // L3: wrong_direction + low_confidence; L4: already_handled;
    // L5: wrong_type; L6: wrong_level; L7: wrong_threshold;
    // L8: wrong_pairing. None belong on the L1 source-candidate
    // surface — L1's distinctive 5th value is ``duplicate``.
...
describe("/lake/source-candidates actions — env-knob readers", () => {
