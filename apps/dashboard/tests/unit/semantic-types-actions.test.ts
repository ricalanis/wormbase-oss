/**
 * Server-action tests for /lake/semantic-types/actions.ts — L5 Sub-wave D.
 *
 ...
 *
 * Concern #1: L5's reject reason enum is L5-distinct. ``wrong_type``
 * replaces L4's ``already_handled`` and L7's ``wrong_threshold``.
 */
import { describe, expect, it } from "vitest";

import { __test__ } from "../../app/(app)/lake/semantic-types/actions";

const { VALID_REJECT_REASONS, readBase, readToken } = __test__;

describe("/lake/semantic-types actions — reject reasons enum", () => {
  it("accepts only the 5 canonical L5 reasons (false_positive / low_value / wrong_type / out_of_scope / other)", () => {
    expect(VALID_REJECT_REASONS.has("false_positive")).toBe(true);
    expect(VALID_REJECT_REASONS.has("low_value")).toBe(true);
    expect(VALID_REJECT_REASONS.has("wrong_type")).toBe(true);
  ...
  it("rejects L3-only, L4-only, and L7-only reasons (defensive — L5 enum is distinct)", () => {
    // L3 ships wrong_direction + low_confidence; L7 ships wrong_threshold;
    // L4 ships already_handled. None of those belong on the L5
    // semantic-types surface.
...
describe("/lake/semantic-types actions — env-knob readers", () => {
