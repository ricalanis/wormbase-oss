/**
 * Server-action tests for /lake/quality/actions.ts — L7 Sub-wave D.
 *
...
import { __test__ } from "../../app/(app)/lake/quality/actions";

const { VALID_REJECT_REASONS, readBase, readToken } = __test__;

describe("/lake/quality actions — reject reasons enum", () => {
  it("accepts only the 5 canonical reasons (false_positive / low_value / wrong_threshold / out_of_scope / other)", () => {
    expect(VALID_REJECT_REASONS.has("false_positive")).toBe(true);
    expect(VALID_REJECT_REASONS.has("low_value")).toBe(true);
    expect(VALID_REJECT_REASONS.has("wrong_threshold")).toBe(true);
  ...
  it("rejects L3-only reasons (defensive — L7 enum is distinct)", () => {
    // The L3 lineage enum carries `wrong_direction` + `low_confidence`;
    // those are NOT valid for L7 quality checks.
...
describe("/lake/quality actions — env-knob readers", () => {
