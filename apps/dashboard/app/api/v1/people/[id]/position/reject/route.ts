/**
 * POST /api/v1/people/[id]/position/reject — admin rejects a worm-proposed position.
 *
 * Wave H Phase 2 Task 2C — Position Auto-Confirm UX.
 *
 * Thin proxy over worm-core's
 * ``POST /api/v1/people/{id}/position/reject``. The current admin Person
 * — resolved server-side from the tenant cookie — becomes
 * ``rejected_by`` on the entry; never trusted from the wire. The
 * optional ``reason`` is forwarded verbatim for trace-UI explainability.
 */
import { NextResponse } from "next/server";
import type { NextRequest } from "next/server";
import { getCurrentPerson } from "../../../../../../../lib/server/identity";
import { rejectPositionProposal } from "../../../../../../../lib/server/worm-core-write";
...
export async function POST(
  ...
  try {
    const result = await rejectPositionProposal(id, {
      tenantSlug: tenant.slug,
      position,
      rejectedBy: me.personId,
      reason,
    });
