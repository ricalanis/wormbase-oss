 ...
 *     and ``POST /api/v1/write_actions/column_classifications_reject/{classification_id}``
 *     (Sub-wave C).
 *
 ...
 *     Surface enforces CLAUDE.md §9's "no self-grant placeholders" rule
 *     for the column-classification audit trail.
 *
 *   * Reject reason is constrained to the strict 5-value L6 enum on
 *     :class:`ColumnClassificationRejectedPayload`: ``false_positive`` /
 *     ``low_value`` / ``wrong_level`` / ``out_of_scope`` / ``other``.
 *     The L6-specific 5th value is ``wrong_level`` (distinct from L5's
 *     ``wrong_type``, L4's ``already_handled`` and L7's
 *     ``wrong_threshold``). The dropdown surfaces 5 options; this
 *     action validates again at the boundary.
 */
"use server";
...
export interface ColumnClassificationActionResult {
  ok: boolean;
  error?: string;
}
 ...
 * fallback (mirroring L3/L7/L4/L5 actions).
 */
...
/**
 * Confirm a previously-proposed column-classification. Forwards to
 * worm-core's write API (L6 Sub-wave C).
 */
export async function confirmColumnClassification(
  classificationId: string,
  notes?: string,
): Promise<ColumnClassificationActionResult> {
  const id = (classificationId ?? "").trim();
  if (!id) return { ok: false, error: "missing classification_id" };
  ...
  if (!base) {
    return {
      ok: false,
      error:
        "confirm_column_classification endpoint unavailable (no WORM_CORE_API_URL configured)",
    };
  }
  ...
  const url = `${base}/api/v1/write_actions/column_classifications_confirm/${encodeURIComponent(id)}`;
  const trimmedNotes =
  ...
  if (res.status === 404) {
    return {
      ok: false,
      error: `no column_classification_proposed entry found for classification_id ${id}`,
    };
  }
  if (!res.ok) {
...
/**
 * Reject a previously-proposed column-classification. Forwards to
 * worm-core's write API (L6 Sub-wave C). ``reason`` is the strict 5-
 * value L6 enum value from the rejection modal dropdown.
 */
export async function rejectColumnClassification(
  classificationId: string,
  reason: string,
  notes?: string,
): Promise<ColumnClassificationActionResult> {
  ...
  if (!base) {
    return {
      ok: false,
      error:
        "reject_column_classification endpoint unavailable (no WORM_CORE_API_URL configured)",
    };
  }
  const token = readToken();
  if (!token) {
  ...
  const tenant = await getTenantFromCookies();
  const url = `${base}/api/v1/write_actions/column_classifications_reject/${encodeURIComponent(id)}`;
  const trimmedNotes =
    typeof notes === "string" && notes.trim().length > 0
      ? notes.trim().slice(0, 2048)
      : undefined;
