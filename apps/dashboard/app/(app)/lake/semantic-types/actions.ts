/**
 * Server actions for /lake/semantic-types (L5 Sub-wave D, 2026-06-05).
 *
 ...
 *     and ``POST /api/v1/write_actions/semantic_types_reject/{type_id}``
 *     (Sub-wave C).
 *
 ...
 *     from ``getCurrentPerson(companyId)`` — never a placeholder. Surface
 *     enforces CLAUDE.md §9's "no self-grant placeholders" rule for
 *     the semantic-types audit trail.
 *
 *   * Reject reason is constrained to the strict 5-value L5 enum on
 *     :class:`SemanticTypeRejectedPayload`: ``false_positive`` /
 *     ``low_value`` / ``wrong_type`` / ``out_of_scope`` / ``other``.
 *     The 5th value is L5-specific: ``wrong_type`` replaces L4's
 *     ``already_handled`` and L7's ``wrong_threshold``. The dropdown
 *     surfaces 5 options; this action validates again at the boundary.
 */
"use server";
...
export interface SemanticTypeActionResult {
  ok: boolean;
  error?: string;
}
 ...
 * fallback (mirroring L3/L7/L4 actions).
 */
...
/**
 * Confirm a previously-proposed semantic type. Forwards to worm-core's
 * write API (L5 Sub-wave C).
 */
export async function confirmSemanticType(
  typeId: string,
  notes?: string,
): Promise<SemanticTypeActionResult> {
  const id = (typeId ?? "").trim();
  if (!id) return { ok: false, error: "missing type_id" };
  ...
  if (!base) {
    return {
      ok: false,
      error:
        "confirm_semantic_type endpoint unavailable (no WORM_CORE_API_URL configured)",
    };
  }
  ...
  const url = `${base}/api/v1/write_actions/semantic_types_confirm/${encodeURIComponent(id)}`;
  const trimmedNotes =
  ...
  if (res.status === 404) {
    return {
      ok: false,
      error: `no semantic_type_proposed entry found for type_id ${id}`,
    };
  }
  if (!res.ok) {
...
/**
 * Reject a previously-proposed semantic type. Forwards to worm-core's
 * write API (L5 Sub-wave C). ``reason`` is the strict enum value from
 * the rejection modal dropdown.
 */
export async function rejectSemanticType(
  typeId: string,
  reason: string,
  notes?: string,
): Promise<SemanticTypeActionResult> {
  ...
  if (!base) {
    return {
      ok: false,
      error:
        "reject_semantic_type endpoint unavailable (no WORM_CORE_API_URL configured)",
    };
  }
  const token = readToken();
  if (!token) {
  ...
  const tenant = await getTenantFromCookies();
  const url = `${base}/api/v1/write_actions/semantic_types_reject/${encodeURIComponent(id)}`;
  const trimmedNotes =
    typeof notes === "string" && notes.trim().length > 0
      ? notes.trim().slice(0, 2048)
      : undefined;
