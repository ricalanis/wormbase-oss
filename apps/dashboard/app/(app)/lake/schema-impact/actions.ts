/**
 * Server actions for /lake/schema-impact (L4 Sub-wave D, 2026-06-02).
 *
 * Architectural contract (mirrors /lake/lineage/actions.ts +
 * /lake/quality/actions.ts):
 *
 ...
 *     and ``POST /api/v1/write_actions/schema_impacts_reject/{impact_id}``
 *     (Sub-wave C).
 *
 ...
 *     schema-impact audit trail.
 *
 *   * Reject reason is constrained to the strict 5-value enum on
 *     :class:`SchemaImpactRejectedPayload` — note: this set is
 *     L4-specific and differs from L3's and L7's enums by one value
 *     each (L4 ships ``already_handled`` + ``low_value``; L3 ships
 *     ``wrong_direction`` + ``low_confidence``; L7 ships
 *     ``wrong_threshold`` + ``low_value``). The dropdown surfaces 5
 *     options; this action validates again at the boundary.
 */
"use server";
...
export interface SchemaImpactActionResult {
  ok: boolean;
  error?: string;
}
...
/**
 * Confirm a previously-proposed schema impact. Forwards to worm-core's
 * write API (L4 Sub-wave C).
 */
export async function confirmSchemaImpact(
  impactId: string,
  notes?: string,
): Promise<SchemaImpactActionResult> {
  const id = (impactId ?? "").trim();
  if (!id) return { ok: false, error: "missing impact_id" };
  ...
  if (!base) {
    return {
      ok: false,
      error:
        "confirm_schema_impact endpoint unavailable (no WORM_CORE_API_URL configured)",
    };
  }
  ...
  const url = `${base}/api/v1/write_actions/schema_impacts_confirm/${encodeURIComponent(id)}`;
  const trimmedNotes =
  ...
  if (res.status === 404) {
    return {
      ok: false,
      error: `no schema_impact_proposed entry found for impact_id ${id}`,
    };
  }
  if (!res.ok) {
...
/**
 * Reject a previously-proposed schema impact. Forwards to worm-core's
 * write API (L4 Sub-wave C). ``reason`` is the strict enum value from
 * the rejection modal dropdown.
 */
export async function rejectSchemaImpact(
  impactId: string,
  reason: string,
  notes?: string,
): Promise<SchemaImpactActionResult> {
  ...
  if (!base) {
    return {
      ok: false,
      error:
        "reject_schema_impact endpoint unavailable (no WORM_CORE_API_URL configured)",
    };
  }
  const token = readToken();
  if (!token) {
  ...
  const tenant = await getTenantFromCookies();
  const url = `${base}/api/v1/write_actions/schema_impacts_reject/${encodeURIComponent(id)}`;
  const trimmedNotes =
    typeof notes === "string" && notes.trim().length > 0
      ? notes.trim().slice(0, 2048)
      : undefined;
