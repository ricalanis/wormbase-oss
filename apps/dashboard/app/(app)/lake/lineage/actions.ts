/**
 * Server actions for /lake/lineage (L3 Sub-wave D, 2026-05-29).
 *
 ...
 *     and ``POST /api/v1/write_actions/lineage_edges_reject/{edge_id}``
 *     (Sub-wave C).
 *
 *   * Admin role check is enforced at the server-action layer per
 *     Sub-wave C concern #3 (defense in depth — worm-core's
 ...
 *     CLAUDE.md §9's "no self-grant placeholders" rule for the lineage
 *     audit trail.
 *
 *   * Reject reason is constrained to the strict enum on
 *     :class:`LineageEdgeRejectedPayload` (5 values). The dropdown
...
export interface LineageActionResult {
  ok: boolean;
  error?: string;
}
 ...
 * fallback (mirroring promoteSemanticGap).
 */
...
/**
 * Confirm a previously-proposed lineage edge. Forwards to worm-core's
 * write API (L3 Sub-wave C).
 */
export async function confirmLineageEdge(
  edgeId: string,
  notes?: string,
): Promise<LineageActionResult> {
  const id = (edgeId ?? "").trim();
  if (!id) return { ok: false, error: "missing edge_id" };
  ...
  if (!base) {
    return {
      ok: false,
      error:
        "confirm_lineage_edge endpoint v1.1 (no WORM_CORE_API_URL configured)",
    };
  }
  ...
  const url = `${base}/api/v1/write_actions/lineage_edges_confirm/${encodeURIComponent(id)}`;
  const trimmedNotes =
  ...
  if (res.status === 404) {
    return {
      ok: false,
      error: `no lineage_edge_proposed entry found for edge_id ${id}`,
    };
  }
  if (!res.ok) {
...
/**
 * Reject a previously-proposed lineage edge. Forwards to worm-core's
 * write API (L3 Sub-wave C). ``reason`` is the strict enum value from
 * the rejection modal dropdown.
 */
export async function rejectLineageEdge(
  edgeId: string,
  reason: string,
  notes?: string,
): Promise<LineageActionResult> {
  ...
  if (!base) {
    return {
      ok: false,
      error:
        "reject_lineage_edge endpoint v1.1 (no WORM_CORE_API_URL configured)",
    };
  }
  const token = readToken();
  if (!token) {
  ...
  const tenant = await getTenantFromCookies();
  const url = `${base}/api/v1/write_actions/lineage_edges_reject/${encodeURIComponent(id)}`;
  const trimmedNotes =
    typeof notes === "string" && notes.trim().length > 0
      ? notes.trim().slice(0, 2048)
      : undefined;
