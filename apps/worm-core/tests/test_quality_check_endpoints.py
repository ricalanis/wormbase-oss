"""L7 Sub-wave C — HTTP-endpoint tests for quality_checks_confirm/reject.
...
async def _seed_proposed_check(
    memory_ledger: InMemoryLedger,
    *,
    check_id: str,
    company_id: UUID | None = None,
) -> None:
    """Seed a ``quality_check_proposed`` execute entry for the tenant.

    The handlers walk the ledger for this exact tool to surface 404
    on unknown checks; seeding a propose-cycle is the canonical setup.
    """
...
async def test_confirm_missing_auth_returns_401(client: TestClient) -> None:
    """No bearer → 401 from the HTTP layer (matches v2.A + L3 pattern)."""
    resp = await client.post(
        "/api/v1/write_actions/quality_checks_confirm/check-1",
...
async def test_reject_missing_auth_returns_401(client: TestClient) -> None:
    ...
    resp = await client.post(
        "/api/v1/write_actions/quality_checks_reject/check-1",
        json={
...
async def test_confirm_writes_ledger_entry(
    ...
    """Successful confirm emits a ``quality_check_confirmed`` execute entry."""
    check_id = "check-confirm-1"
    await _seed_proposed_check(memory_ledger, check_id=check_id)
    company_id = tenant_to_uuid(TENANT_SLUG)
    admin = uuid4()

    resp = await client.post(
        f"/api/v1/write_actions/quality_checks_confirm/{check_id}",
    ...
    assert "emit_quality_check_confirmed" in confirmed_tools


async def test_reject_writes_ledger_entry(
    client: TestClient, memory_ledger: InMemoryLedger,
) -> None:
    """Successful reject emits a ``quality_check_rejected`` execute entry."""
    check_id = "check-reject-1"
    await _seed_proposed_check(memory_ledger, check_id=check_id)
    company_id = tenant_to_uuid(TENANT_SLUG)
    admin = uuid4()

    resp = await client.post(
        f"/api/v1/write_actions/quality_checks_reject/{check_id}",
        headers=_auth_headers(),
        json={
            "company_id": str(company_id),
            "rejected_by": str(admin),
            "reason": "false_positive",
            "notes": "schema-pattern heuristic was misleading",
        },
    )
    assert resp.status == 200, await resp.text()
    body = await resp.json()
    ...
    assert "emit_quality_check_rejected" in tools


async def test_confirm_404_on_unknown_check_id(
    client: TestClient,
) -> None:
    """Check not in the projection → 404 from the confirm handler."""
    company_id = tenant_to_uuid(TENANT_SLUG)
    resp = await client.post(
        "/api/v1/write_actions/quality_checks_confirm/unknown-check",
        headers=_auth_headers(),
        json={
            "company_id": str(company_id),
...
async def test_reject_404_on_unknown_check_id(
    client: TestClient,
) -> None:
    """Check not in the projection → 404 from the reject handler."""
    company_id = tenant_to_uuid(TENANT_SLUG)
    resp = await client.post(
        "/api/v1/write_actions/quality_checks_reject/unknown-check",
        headers=_auth_headers(),
        json={
            "company_id": str(company_id),
            "rejected_by": str(uuid4()),
...
async def test_reject_400_on_invalid_reason(
    ...
    check_id = "check-reject-bad-reason"
    await _seed_proposed_check(memory_ledger, check_id=check_id)
    company_id = tenant_to_uuid(TENANT_SLUG)
