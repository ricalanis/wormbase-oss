"""v022 — create ``projection_quality_checks``.

L7 lake-side compounding loop folded view of quality checks proposed
by inference strategies (schema_pattern / dbt_tests / historical_stats)
and confirmed/rejected by admin operators.

Folded from three ledger entry kinds (additive per schema-evolution
doctrine Rule 2; KIND_REGISTRY 111 → 114):

* ``quality_check_proposed`` → INSERT (or UPDATE evidence/confidence
  on re-proposal of the same ``check_id`` before resolution)
...
because the deterministic ``(company_id, check_id)`` primary key
collapses re-proposal onto the same row regardless of replay order.

Schema invariants:

* ``check_id`` is a deterministic hash of
  ``(table_id, column, check_kind, config)`` minted upstream by the
  L7 inference service; the projection treats it as opaque.
...
* ``confidence`` is a Float in [0.0, 1.0]; the payload-side validator
  ...
  projection.
* ``column`` may be NULL to express a table-level check (common for
  ``row_count_range`` or ``freshness``).
...
pattern from v014/v015/v016/v021. ``config`` and ``evidence`` are
...
_TABLE_NAME: str = "projection_quality_checks"


def _build_table() -> Table:
    """Construct the projection_quality_checks Table on a fresh MetaData.
    ...
    is already defined`` (mirrors the v016/v021 pattern). Production
    runs apply once per boot per engine, so the fresh MetaData has no
    runtime cost.

    PK is composite ``(company_id, check_id)`` per spec §3.6; the
    pair is unique per tenant so re-proposal of the same logical
    check folds onto the same row.
    """
    metadata = MetaData()
    return Table(
        _TABLE_NAME,
        ...
        CheckConstraint(
            ...
            name="ck_projection_quality_checks_state",
        ),
        Index(
            "ix_projection_quality_checks_state",
            "company_id",
            "state",
        ),
        Index(
            "ix_projection_quality_checks_table",
            "company_id",
            "table_id",
        ),
        Index(
            "ix_projection_quality_checks_kind",
            "company_id",
            "check_kind",
        ),
    )
...
def _create(sync_conn: Any) -> None:
    """Idempotent CREATE TABLE for projection_quality_checks.
...
class Migration:
    version: int = 22
    description: str = (
        "create projection_quality_checks — L7 lake-side compounding-loop "
        "folded view of quality checks (proposed/confirmed/rejected); "
        "composite PK (company_id, check_id)"
    )
