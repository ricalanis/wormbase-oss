"""Presence: the conversation contract for chat-presence.
...
becomes a chat_received entry, regardless of whether the worm speaks. This
invariant is upheld at the channel-adapter level (per spike §C8); Presence
is read-only on this axis.

Renamed from ConversationContract → Presence in chat-worm Wave B; the
old name is exported as an alias for legacy imports.
"""
...
from wormbase_chat_presence.relevance import RulesBasedRelevanceGate, Talkativeness
...
class Presence:
    """The conversation contract for chat-presence.

    Wraps RulesBasedRelevanceGate + InterjectionGate to render the
    speech-act decision (`should_speak`) and the digest tick. Lurker
    semantics live at the channel-adapter; Presence reads talkativeness
    via the wired RelevanceGate.

    Same constructor signature + body as the legacy ConversationContract;
    no behavior changes in this lift.
    """
...
# Legacy alias for callers that imported ConversationContract.
ConversationContract = Presence


__all__ = ["ConversationContract", "Presence"]
