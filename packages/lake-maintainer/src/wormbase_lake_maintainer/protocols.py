...
the existing Connector Protocol lifted to a per-instance object; only
external + filedrop families implement it. MaintainableSource carries
the four maintenance methods and is implemented by all four families.
LakeMaintainer iterates the union typed as MaintainableSource.

Per spike §8 C2: ``watch()`` is NOT on AcquirableSource. Empirically
every existing connector returns an empty async iterator; CDC moves to
an optional capability flag if a real consumer ever appears.
"""
...
class AcquirableSource(Protocol):
    ...
    Implementations wrap a ``wormbase_connectors.Connector`` driver plus
    per-instance metadata (id, classification, domain, owner). The
    Connector is the *driver*; AcquirableSource is the *instance*.
    """
...
class LakeStore(Protocol):
    ...
    > actually accepts. The Connectors-package ``Profile`` is a different
